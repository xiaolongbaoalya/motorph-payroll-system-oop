...
public class RequestService {
    ...
    private static final int REQUEST_ID_WIDTH = 5;
    ...
    public List<Request> getEmployeeRequestHistory(int employeeNumber, Session session) throws InvalidAccessException {
        AccessControlService.requireRequestHistoryAccess(session, employeeNumber, "Read employee request history");
    ...
    private String generateUniqueRequestId() {
        int nextSequence = requestDAO.getAll().stream()
        ...
        String candidate;
        do {
            candidate = formatRequestId(nextSequence++);
        } while (requestDAO.findById(candidate) != null);
        return candidate;
    }

    private int extractNumericSuffix(String requestId) {
        if (requestId == null) {
            return -1;
        }

        String trimmed = requestId.trim().toUpperCase(Locale.ROOT);
        if (!trimmed.startsWith(REQUEST_ID_PREFIX)) {
            return -1;
        }

        String suffix = trimmed.substring(REQUEST_ID_PREFIX.length()).trim();
    ...
    private String formatRequestId(int sequence) {
        return REQUEST_ID_PREFIX + String.format(Locale.ROOT, "%0" + REQUEST_ID_WIDTH + "d", sequence);
    }
