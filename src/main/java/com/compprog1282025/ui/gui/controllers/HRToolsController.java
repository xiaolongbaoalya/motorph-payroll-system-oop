...
public class HRToolsController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (!AccessControlService.canAccessHrPanel(session)) {
    ...
    private void loadTeamAttendance() {
        ...
        try {
            List<Attendance> rows = teamAttendanceController.loadTeamAttendance(month, year, currentSession());
    ...
    private void loadTeamRequests() {
        try {
            tblPendingLeave.setItems(FXCollections.observableArrayList(teamRequestsController.getPendingLeaveRequests(currentSession())));
            tblRequestHistory.setItems(FXCollections.observableArrayList(teamRequestsController.getRequestHistory(currentSession())));
        } catch (Exception e) {
            showAlert("Access Denied", e.getMessage());
        }
    }
