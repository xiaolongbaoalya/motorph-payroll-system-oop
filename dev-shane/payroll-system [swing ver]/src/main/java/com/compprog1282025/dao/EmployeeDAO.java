...
public class EmployeeDAO implements DAO<Employee, Integer> {
    ...
    private void appendArchivedEmployee(Employee employee) {
        ...
        try (CSVWriter writer = new CSVWriter(new FileWriter(archiveFile, true))) {
            if (writeHeader) {
                writer.writeNext(EMPLOYEE_HEADERS);
            }
    ...
    private File resolveArchiveFile() {
        File employeeFile = new File(EMPLOYEE_CSV_PATH).getAbsoluteFile();
        File baseDir = employeeFile.getParentFile();
        if (baseDir != null) {
            return new File(baseDir, "archived_employees.csv");
        }
        return new File(ARCHIVED_EMPLOYEE_CSV_PATH).getAbsoluteFile();
    }
