...
public class FinanceService implements Calculable {
    ...
    public double calculateMonthSalary(Employee employee, int year, int month) {
    ...
    // Terminal/backend-secured variant for role-gated finance operations.
    public double calculateMonthSalary(Employee employee, int year, int month, Session session) throws InvalidAccessException {
        requireFinancePermission(session, Permission.CALCULATE_SALARY);
        return calculateMonthSalary(employee, year, month);
    }

    public PayslipComputation computePayslip(Employee employee, int year, int month) {
        if (employee == null) {
            return null;
        }

        double basicSalary = calculateMonthSalary(employee, year, month);
        if (basicSalary <= 0) {
            return null;
        }
    ...
    // Terminal/backend-secured variant for role-gated payslip generation.
    public PayslipComputation computePayslip(Employee employee, int year, int month, Session session) throws InvalidAccessException {
        requireFinancePermission(session, Permission.GENERATE_PAYSLIP);
        return computePayslip(employee, year, month);
    }

    private void requireFinancePermission(Session session, Permission permission) throws InvalidAccessException {
        if (session == null || session.getUser() == null) {
            throw new InvalidAccessException("No active session.");
        }
