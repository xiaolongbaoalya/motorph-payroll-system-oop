...
public class RequestService {
    ...
    // Legacy no-session overload kept for compatibility. Use session overload for RBAC.
    ...
    public void approveRequest(String requestId, Session approverSession) throws InvalidAccessException {
        validateApprovalPermission(approverSession);
    ...
    private void validateApprovalPermission(Session approverSession) throws InvalidAccessException {
        if (approverSession == null || approverSession.getUser() == null) {
            throw new InvalidAccessException("No active session.");
        }

        Role role = approverSession.getUser().getRole();
        if (role == null || !role.hasPermission(Permission.APPROVE_LEAVE)) {
