...
public class RequestSectionController {
    ...
    public void submitRequest(Session session) {
        ...
        // RequestService will generate a collision-safe ID when this is blank.
        newReq.setRequestId("");
        ...
        try {
            requestService.submitRequest(newReq, session);
            refreshRequestHistory(session);
            alert.accept("Success", "Your leave request has been filed.");
        } catch (Exception e) {
            alert.accept("Access Denied", e.getMessage());
        }
    }
    ...
    private void refreshRequestHistory(Session session) {
        if (session != null && session.getUser() != null) {
            ...
            try {
                List<Request> myRequests = requestService.getEmployeeRequestHistory(empId, session);
